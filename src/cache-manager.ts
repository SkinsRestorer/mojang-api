...
export interface CacheManager {
  ...
  putUUIDToSkin: (uuid: string, skinProperty: SkinProperty | null, createdAt: Date) => void;
  getUUIDToSkin: (uuid: string) => Promise<DatabaseResult<SkinProperty> | undefined>;
  close: () => void;
}
...
export function createCacheManager(): CacheManager {
  ...
  return {
    ...
    async getNameToUUID(name: string): Promise<DatabaseResult<string> | undefined> {
      return nameToUUIDCache.get(name.toLowerCase());
    },

    putUUIDToSkin(uuid: string, skinProperty: SkinProperty | null, createdAt: Date) {
    ...
    async getUUIDToSkin(uuid: string): Promise<DatabaseResult<SkinProperty> | undefined> {
